...
public class CMenu {
    ...
    public static void loadFromDB(Context context) {
        CDatabase db = new CDatabase(context);
        Cursor c = db.select("select menuname, part1, part2, dishname, price, comment, dishcode, print1, print2, store from menu");
        SortedSet<String> p1 = new TreeSet<>();
        SortedSet<String> p2 = new TreeSet<>();
        SortedSet<String> m = new TreeSet<>();
        while (c.moveToNext()) {
            ...
            d.id = c.getString(6);
            d.print1 = c.getString(7);
            d.print2 = c.getString(8);
            d.store = c.getString(9);
            p1.add(d.part1);
            p2.add(d.part2);
            m.add(d.menu);
            listOfDishes.add(d);
